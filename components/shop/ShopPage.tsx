...
import {
  ...
  Shirt, Beaker, Star, ChevronLeft, Dices, Plus, Minus
} from "lucide-react";
...
export function ShopPage({ characterId, moneyPc, items, characterClass }: Props) {
  ...
  // ─── Carrinho ─────────────────────────────────────────────
  function addToCart(item: ItemRow, improvements = 0, isArcanium = false, arcaniumCircle?: number) {
    setCart(prev => {
      const existing = prev.find(
        e => e.item.slug === item.slug &&
  ...
  const configPrice = selectedItem
    ? priceWithArcanium(selectedItem.price_pc, configImprovements, configArcanium ? configArcaniumCircle : undefined)
    : 0;
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,#f5c86a_0,#f6ead0_40%,#efe1bd_100%)]">
      ...
      <div className="sticky top-0 z-30 bg-stone-950/95 backdrop-blur border-b border-stone-800 px-4 py-3 flex items-center gap-3">
        <button onClick={() => router.back()} className="rounded-full p-2 hover:bg-stone-800 transition text-stone-400 hover:text-white">
          <ChevronLeft size={20} />
        </button>
        <div className="flex-1">
          <p className="text-xs text-stone-500 font-medium">Mercador Aldric</p>
          <p className="text-sm font-black text-amber-200 leading-tight">"Bem-vindo, viajante. O que procura hoje?"</p>
        </div>
        <div className="text-right">
          <p className="text-xs text-stone-500">Seu saldo</p>
          <p className="text-sm font-black text-amber-400">{formatMoney(currentMoney)}</p>
        </div>
        <button
          onClick={() => setShowCart(true)}
          className="relative rounded-full p-2.5 bg-amber-800 hover:bg-amber-700 transition text-amber-50"
        >
      ...
      <div className="max-w-2xl mx-auto px-4 py-5 space-y-4">
        ...
        {/* Lista de itens */}
        <p className="text-xs text-stone-500">{filtered.length} {filtered.length === 1 ? "item" : "itens"}</p>
              ...
              <div className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-stone-500 truncate">
                    {item.weapon_damage_dice && `${item.weapon_damage_dice} · ${item.weapon_critical} · `}
                ...
                <div className="text-right">
                  ...
                  <button
                    onClick={() => { setSelectedItem(item); setConfigImprovements(0); setConfigArcanium(false); }}
          ...
          <div className="w-full sm:max-w-sm bg-white rounded-2xl shadow-2xl overflow-hidden">
            <div className="bg-amber-900 px-5 py-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-amber-300 text-xs">{formatMoneyPP(selectedItem.price_pc)} base</p>
              </div>
              <button onClick={() => setSelectedItem(null)} className="text-amber-300 hover:text-white transition">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              {["arma","armadura","escudo","esoterico"].includes(selectedItem.category) && (
                <div>
                  <p className="text-xs font-bold uppercase tracking-wider text-stone-500 mb-2">Item Superior</p>
                  <div className="flex items-center gap-3">
                    <button onClick={() => setConfigImprovements(i => Math.max(0, i-1))} className="w-8 h-8 rounded-lg bg-stone-100 font-bold text-stone-700 hover:bg-stone-200 transition"><Minus size={12} /></button>
                    <span className="flex-1 text-center font-bold">{configImprovements} melhoria{configImprovements !== 1 ? "s" : ""} {configImprovements === 4 ? "(obra-prima)" : ""}</span>
                    <button onClick={() => setConfigImprovements(i => Math.min(4, i+1))} className="w-8 h-8 rounded-lg bg-stone-100 font-bold text-stone-700 hover:bg-stone-200 transition"><Plus size={12} /></button>
                  </div>
                  <p className="text-xs text-stone-400 mt-1 text-center">Cada melhoria: +50% no preço</p>
                </div>
              )}
              ...
              {["arma","armadura","escudo"].includes(selectedItem.category) && (
                <div>
                  <div className="flex items-center gap-2">
                    <input type="checkbox" id="arcanium" checked={configArcanium} onChange={e => setConfigArcanium(e.target.checked)} className="accent-amber-700" />
                    <label htmlFor="arcanium" className="text-sm font-bold text-stone-700">Metal Arcanium</label>
                  </div>
                  {configArcanium && (
                    <div className="mt-2">
                      ...
                      <div className="flex gap-1">
                        {[1,2,3,4,5].map(c => (
                          <button key={c} onClick={() => setConfigArcaniumCircle(c)}
                            className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition ${configArcaniumCircle === c ? "bg-purple-700 text-white" : "bg-stone-100 text-stone-600 hover:bg-stone-200"}`}
                          >{c}º</button>
                        ))}
                      </div>
                    </div>
              ...
              <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 flex justify-between items-center">
                ...
                <span className="font-black text-amber-900 text-lg">{formatMoneyPP(configPrice)}</span>
              </div>

              {currentMoney < configPrice && (
                <p className="text-xs text-red-600 text-center">Saldo insuficiente para este item.</p>
              )}

              <button
                disabled={currentMoney < configPrice}
                onClick={() => addToCart(selectedItem, configImprovements, configArcanium, configArcanium ? configArcaniumCircle : undefined)}
          ...
          <div className="w-full sm:max-w-sm bg-white rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            <div className="bg-stone-950 px-5 py-4 flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-2 text-amber-50">
                ...
                <p className="font-black">Carrinho ({cart.reduce((s,e)=>s+e.quantity,0)} itens)</p>
              </div>
              <button onClick={() => setShowCart(false)} className="text-stone-400 hover:text-white"><X size={18} /></button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-stone-100">
              {cart.length === 0 ? (
                ...
                const unitPrice = priceWithArcanium(entry.item.price_pc, entry.improvements, entry.arcaniumCircle);
                return (
                  <div key={i} className="flex items-center gap-3 px-5 py-3">
                    <div className="flex-1">
                      <p className="font-bold text-stone-900 text-sm">{entry.item.name}</p>
                    ...
                    <p className="font-black text-amber-800 text-sm">{formatMoneyPP(unitPrice * entry.quantity)}</p>
                    <button onClick={() => removeFromCart(i)} className="text-stone-400 hover:text-red-500 transition"><X size={14} /></button>
                  </div>
                );
              })}
            </div>
            ...
            <div className="border-t border-stone-200 px-5 py-4 space-y-3 flex-shrink-0">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className={`font-black ${canAfford ? "text-emerald-700" : "text-red-600"}`}>
                  {formatMoney(currentMoney - cartTotal)}
                </span>
              </div>
              {!canAfford && <p className="text-xs text-red-600 text-center">Saldo insuficiente.</p>}
      ...
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 rounded-2xl px-5 py-3 text-sm font-semibold text-amber-50 shadow-xl"
